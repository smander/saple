...
- **Banner reads "SAPLE - Formal Verification Framework".**
- **All user-facing CLI strings renamed** from SABLE / sable / SBL to SAPLE / saple / SPL: `saple --version` now reports `saple 0.2.0`, help pages, progress messages, generated-file headers (`Generated by saple translate`), and trace log lines. Unchanged on purpose: the `.sable-trace/` run directory, the `SABLE_PAO_LEGACY` variable, and the `sable_version` key in trace manifests.
- The harness's default binary path now points at `target/release/saple`.
- **Packages rebuilt** with Z3 4.16.0 statically linked. The binary's internal version string remains 0.2.0.
